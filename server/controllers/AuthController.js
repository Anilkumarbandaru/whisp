...
export const signup = async (request, response, next) => {
    try {
        ...
        if(!email || !password){
            return request.status(400).send("Email abd Password is required")
        }
...
export const login = async (request, response, next) => {
    try {
        const { email, password } = request.body;
        if(!email || !password){
            return response.status(400).send("Email abd Password is required")
        }
...
export const addProfileImage = async (request, response, next) => {
    ...
    try {
        ...
        const date = Date.now();
        const fileName = date + request.file.originalname;
